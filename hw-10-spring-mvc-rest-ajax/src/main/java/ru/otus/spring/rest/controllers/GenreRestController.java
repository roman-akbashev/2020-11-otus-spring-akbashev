...
public class GenreRestController {
    ...
    public ResponseEntity<List<GenreDto>> getGenres() {
        return new ResponseEntity<>(genreService.getAll().stream().map(mapper::toDto).collect(Collectors.toList()), HttpStatus.OK);
    }

    @GetMapping("/api/genres/{id}")
    public ResponseEntity<GenreDto> getGenre(@PathVariable String id) {
        return new ResponseEntity<>(mapper.toDto(genreService.getById(id)), HttpStatus.OK);
    }

    @PutMapping("/api/genres/{id}")
    public ResponseEntity<GenreDto> editGenre(@PathVariable String id, @RequestBody GenreDto genreDto) {
        return new ResponseEntity<>(mapper.toDto(genreService.update(id, genreDto.getName())), HttpStatus.OK);
    }
    ...
    public ResponseEntity<GenreDto> createGenre(@RequestBody GenreDto genreDto) {
        return new ResponseEntity<>(mapper.toDto(genreService.create(genreDto.getName())), HttpStatus.CREATED);
    }
}
